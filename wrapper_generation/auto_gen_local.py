...
# The output directory will be named after the FILE_PATH argument (e.g., "as6s_app").
...
for command in commands:
    if "fifo" in command:
        # Construct the new file path inside the local directory
        file_name = f"{MODULE_NAME}_1r1w_{RAM_DEPTH}x{DATA_WIDTH}_{command}_wrapper.v"
        o_file_path = os.path.join(output_dir, file_name)
        
        print(f"Generating FIFO wrapper: {o_file_path}")
...
for command in commands:
    if "ram" in command:
        ...
        print(f"Generating RAM wrapper: {o_file_path}")
        with open(o_file_path, mode="w") as fp:
